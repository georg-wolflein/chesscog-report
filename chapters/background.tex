...
\end{equation}
Here, $\vf$ actually represents a \gls{pmf} where the $i$\textsuperscript{th} component $\hat{y}_i$ of the output vector $\vyhat$ represents the probability that the input sample $\vx$ belongs to the class $i\in \sC$.
It follows that
\begin{equation}
    \label{eq:pmf}
...
In this model, if there exists no link between unit $i$ and a particular $j$ then simply $w_{i,j}=0$ and therefore $j$ does not contribute to $i$'s excitation. 
    ...
    \begin{subfigure}{.45\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[blue,domain=-1:0] {0};
            \end{axis}
        \end{tikzpicture}
        \caption{\Glsentrylong{relu}}
        \label{fig:activation_functions_relu}
    \end{subfigure}
    \caption{Plots of the two most common activation functions.}
...
\begin{figure}
    ...
    \label{fig:mlp}
\end{figure}
It can clearly be seen that because \glspl{mlp} are simply nested \glspl{slp}, they still form \glspl{dag} and thus are feedforward networks.
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \end{axis}
    \end{tikzpicture}
    \caption[Illustration of gradient descent training on an error-weight surface.]{Illustration of gradient descent training on an error-weight surface with only one parameter (not drawn to scale). Gradient descent minimises the objective loss function and converges to a local minimum. In practice, deep neural networks have millions of parameters, resulting in a high-dimensional error surface, and local minima often turn out to be `good enough' in that their error is only slightly higher than the global minimum \cite{lecun2015}.}
...
For simplicity, I use the \gls{mse}, although the reader should note that the cross-entropy loss is preferred for classification problems.
...
The loss can be expressed in terms of the weights and biases by combining \cref{eq:slp,eq:mse} as
\begin{equation}
...
To apply backpropagation in \glspl{mlp}, the first step is to compute the gradient of the loss with resepect to each trainable parameter (i.e.\ each layer's weights and biases).
In other words, it is necessary to compute $\frac{\partial L}{\partial \mW_l}$ and $\frac{\partial L}{\partial \vb_l}$ for each layer $l=1,2,\dots,L$, but the exact derivation is left as an exercise to the reader.
...
Similar to \cref{eq:backprop_update_weights,eq:backprop_update_bias}, once the gradients are computed, the weights and biases are updated by applying the rule
\begin{align}
...
Both terms refer to a mathematical operation where a matrix (known as \emph{filter} or \emph{kernel}) is slid over the input image and the sum of products is computed at each location.
...
Similarly, subsequent layers' neurons connect only to a square region of neurons in the respective previous layer, so that the network begins to form a hierarchy of filters.
...
Coming back to examining the first layer of a \gls{cnn}, notice that it is associated with a filter $\mF$. 
The output of the layer is obtained by sliding a $k \times k$ window over the input image, and at each location of the window, the convolution $\mP * \mF$ is computed where $\mP$ are the pixels in the current window.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[Illustration of a convolution operation with a $2\times 2$ kernel.]{Illustration of a convolution operation with a $2\times 2$ kernel. The bias $b$ is added to the output on the right before pointwise applying the activation function $g$ in order to obtain the activations.}
    \label{fig:conv_simple}
\end{figure}
The result is a matrix, too, and so the output of the layer is obtained by applying the nonlinear activation function $g$ pointwise to that matrix plus a bias $b$.
...
Thus, the first convolutional layer needs three filters: one for each input colour channel.
...
\begin{itemize}
    ...
    \item the strideby how many pixels the sliding window is moved each time (\cref{fig:conv_simple,fig:conv_multiple_channels} use $s=1$); and
...
Furthermore, each filter is associated with a bias that is trainable, so the total number of trainable parameters is 
$c_\text{in} \cdot c_\text{out} \cdot (k^2+1)$.
In contrast to \glspl{mlp}, this is independent of the input's spatial size ($h_\text{in} \times w_\text{in}$), and since the value of $k$ is typically quite small, the number of trainable parameters is orders of magnitude smaller than that of a layer in a \gls{mlp}.
...
Typically, a \gls{cnn} consists of a number of convolutional and pooling layers followed by a small \gls{mlp} that has one or two fully-connected layers.
...
They are used immediately after convolutional layers and typically occur less often than convolutional layers in a \gls{cnn} architecture because their excessive use prevents the network from learning any useful features.
...
\begin{enumerate*}[label=(\roman*)]
    \item training \glspl{cnn} for classifying chess squares and pieces in \cref{sec:occupancy_classification,sec:piece_classification}; and
    \item fine-tuning these models in order to adapt to previously unseen chess sets in \cref{chap:adapting}.
\end{enumerate*}

Training a deep \gls{cnn} from scratch (starting from a randomly initialised weight state) can take several days even on modern \glspl{gpu}. 
...
\Cref{fig:transfer_learning} shows which layers are copied and trained using a simple example.
\begin{figure}
    \centering
    \begin{tikzpicture}[
        node distance = 1cm,
        ...
        \node (orig_in) [layer] {input image};
        \node (orig_c1) [layer,below=.75cm of orig_in] {conv \#1};
        \node (orig_c2) [layer,below of=orig_c1] {conv \#2};
        \node (orig_p1) [layer,below of=orig_c2] {pool \#1};
        \node (orig_c3) [layer,below of=orig_p1] {conv \#3};
        \node (orig_re) [layer,below=1cm of orig_c3] {reshape};
        \node (orig_f1) [layer,below of=orig_re] {fc \#1};
        \node (orig_f2) [layer,below of=orig_f1] {fc \#2};
        \node (orig_f3) [layer,below of=orig_f2] {fc \#3};
        \node (orig_ou) [layer,below=.75cm of orig_f3] {output};

        \node[above=0cm of orig_in] {pre-trained model};

        \draw[arrow] (orig_in) -- (orig_c1);
        \draw[arrow] (orig_c1) -- (orig_c2);
        \draw[arrow] (orig_c2) -- (orig_p1);
        \draw[arrow] (orig_p1) -- (orig_c3);
        \draw[arrow] (orig_c3) -- (orig_re) node[midway,fill=white] {$\cdots$};
        \draw[arrow] (orig_re) -- (orig_f1);
        \draw[arrow] (orig_f1) -- (orig_f2);
        \draw[arrow] (orig_f2) -- (orig_f3);
        \draw[arrow] (orig_f3) -- (orig_ou);

        \node (tran_in) [layer,right=4cm of orig_in] {input image};
        \node (tran_c1) [layer,below=.75cm of tran_in] {conv \#1};
        \node (tran_c2) [layer,below of=tran_c1] {conv \#2};
        \node (tran_p1) [layer,below of=tran_c2] {pool \#1};
        \node (tran_c3) [layer,below of=tran_p1] {conv \#3};
        \node (tran_re) [layer,below=1cm of tran_c3] {reshape};
        \node (tran_f1) [layer,below of=tran_re] {fc \#1};
        \node (tran_f2) [layer,below of=tran_f1] {fc \#2};
        \node (tran_ou) [layer,below=.75cm of tran_f2] {output};

        \node[above=0cm of tran_in] {new model};

        \draw[arrow] (tran_in) -- (tran_c1);
        \draw[arrow] (tran_c1) -- (tran_c2);
        \draw[arrow] (tran_c2) -- (tran_p1);
        \draw[arrow] (tran_p1) -- (tran_c3);
        \draw[arrow] (tran_c3) -- (tran_re) node[midway,fill=white] {$\cdots$};
        \draw[arrow] (tran_re) -- (tran_f1);
        \draw[arrow] (tran_f1) -- (tran_f2);
        \draw[arrow] (tran_f2) -- (tran_ou);

        \draw[darrow] (orig_c1) -- (tran_c1) node[midway,fill=white] {\footnotesize copy};
        \draw[darrow] (orig_c2) -- (tran_c2) node[midway,fill=white] {\footnotesize copy};
        \draw[darrow] (orig_p1) -- (tran_p1) node[midway,fill=white] {\footnotesize copy};
        \draw[darrow] (orig_c3) -- (tran_c3) node[midway,fill=white] {\footnotesize copy};
        \draw[darrow] (3.5, 0 |- tran_f1) node[fill=white] {\footnotesize random init} -- (tran_f1);
        \draw[darrow] (3.5, 0 |- tran_f2) node[fill=white] {\footnotesize random init} -- (tran_f2);

        \draw [decorate,decoration={brace,amplitude=10pt}]
            ($(tran_c1)+(2,.375)$) -- ($(tran_re)+(2,.875)$)
            node [black,midway,right,xshift=10pt] {\footnotesize freeze};
        \draw [decorate,decoration={brace,amplitude=10pt}]
            ($(tran_f1)+(2,.375)$) -- ($(tran_f2)+(2,-.375)$)
            node [black,midway,right,xshift=10pt] {\footnotesize train};
    \end{tikzpicture}
    \caption[Illustration of the transfer learning process for domain adaption in deep neural networks.]{Illustration of the transfer learning process for domain adaption in deep neural networks. The boxes represent different types of layers: convolutional (conv), pooling (pool), and fully connected (fc). In the first instance, training proceeds only in the classification head, while the other layers are frozen. Then, the whole network is trained.}
