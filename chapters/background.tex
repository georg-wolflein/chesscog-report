...
In other words, we must compute $\frac{\partial L}{\partial \mW_l}$ and $\frac{\partial L}{\partial \vb_l}$ for each layer $l=1,2,\dots,L$, but the exact derivation is left as an exercise to the reader.
...
This becomes impractical for large datasets due to memory consumption and computational complexity, so we commonly split the dataset into batches and estimate the gradients for each batch in a process known as \emph{stochastic gradient descent}.
...
Coming back to our examination of the first layer of a \gls{cnn}, we know that it is associated with a filter $\mF$. 
...
Having said that, we can finally provide evidence for the claim at the beginning of \cref{sec:cnns} that \glspl{cnn} require significantly fewer trainable parameters than \glspl{mlp}.
...
Since the output of the convolutional/pooling layers is a tensor of rank three (more specifically, we have $c_\text{out}$ feature maps, each of size $h_\text{out} \times w_\text{out}$), it must first be flattened so that all elements are concatenated into a vector.
...
We often set $s=k=2$ and use no padding in order to halve the spatial dimensions (the calculation from \cref{eq:cnn_output_size} holds for pooling layers as well).
...
Then, we can train the classification head in isolation by \emph{freezing}\footnote{Freezing a layer means that the weights and biases in that layer are not updated during backpropagation.} the weights in all the other layers.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption[Illustration of the transfer learning process for domain adaption in deep neural networks.]{Illustration of the transfer learning process for domain adaption in deep neural networks. The boxes represent different types of layers: convolutional (conv), pooling (pool), and fully connected (fc). In the first instance, we train only the classification head, while the other layers are frozen. Then, we train the whole network.}
