            ...
            \ForAll{$s_x \in \{1, 2, \dots, 8\}$}
                \ForAll{$s_y \in \{1, 2, \dots, 8\}$}
                    ...
                    \ForAll{$\vp \in \sP$} \Comment{iterate over all intersection points}
                        ...
                        \EndIf
                        \State $\epsilon \gets \norm{\vp' - \vq}_1$ \Comment{calculate the error}
...
% - replace foor loop on line 16 with matrix operation
% - calculate homography only once per iteration for s_x=s_y=1, then scale it
% - loops on lines 9 and 10 do not need to be nested; the error can be calculated first for the x-direction and then the y-direction and summed (for best x-scale and best y-scale)
% Explain how how final hommography matrix is calculated
