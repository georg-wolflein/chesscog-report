...
In the first step of the algorithm, we would like to detect the majority of these lines and find their intersection points.
...
Therefore, we split them into horizontal and vertical lines and then eliminate similar lines.
...
\begin{enumerate}
    ...
    \item \label{enum:ransac_2}
        ...
        This is the set of inliers, and if the size of this set is greater than the inliers discovered in the previous iteration, we retain this model instead of the previous.
        Repeat from step 1 until reaching a specified number of iterations.
\end{enumerate}

Applied to the problem at hand, choosing any four intersection points in step \ref{enum:ransac_1} would be quite unwise because we would not know which points they represent on the chessboard, making it infeasible to map them to the warped image such that the chess squares are snapped to the grid of whole numbers.
