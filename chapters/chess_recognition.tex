...
\begin{enumerate}
    \item Randomly sample a set of observations from the dataset that contains the minimum number of samples sufficient to fit the model. 
        We require four points to compute a homography, so this is the number of intersection points we must sample in this step.
    \item Fit the model to these samples, and then determine which observations are explained by the model within some threshold (the threshold should be just large enough to account for the expected noise in the dataset). 
...
\end{enumerate}
We must pay attention to some subtleties that arise 

\begin{algorithm}
    \begin{algorithmic}
        \Require the set of intersection points $\sP$
        \State $\sP \gets \{h(\vp) : \vp \in \sP\}$ \Comment{convert to homogenous coordinates}
        \State $i \gets 0$ \Comment{number of iterations}
        \State $\sI \gets \emptyset$ \Comment{set of inliers}
        \While{$(i < i_\text{min}) \vee (\abs{\sI} < c)$}
            ...
            \ForAll{$s_x \in \{1, 2, \dots, 8\}$}
                \ForAll{$s_y \in \{1, 2, \dots, 8\}$}
                    ...
                    \State $\sC = \emptyset$ \Comment{candidate set of inliers}
                    \ForAll{$\vp \in \sP$} \Comment{iterate over all intersection points}
                        \State $\vp' \gets h^{-1}(\mH \vp)$ \Comment{warp the point according to our model}
                        \State $\vp' \gets \vp' \odot \begin{bmatrix}
                            \displaystyle \frac{1}{s_x} & \displaystyle \frac{1}{s_y}
                        \end{bmatrix}^\top$ \Comment{undo scaling}
                        \State $\epsilon \gets \norm{\vp' - \text{round}(\vp')}$ \Comment{calculate the error}
                        \If{$\epsilon \leq \epsilon_\text{max}$}
                            \State $\sC \gets \sC \cup \{\text{round}(\vp')\}$
                        \EndIf
                    \EndFor
                    \If{$\abs{\sC} > \abs{\sI}$}
                        \State $\sI \gets \sC$
        ...
        \EndWhile
    \end{algorithmic}
    \caption{\acs{ransac} for finding the optimal homography.}
\end{algorithm}
