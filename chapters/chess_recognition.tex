...
Applied to our problem, choosing any four intersection points in step \ref{enum:ransac_1} would be quite unwise because we would not know which points they represent on the chessboard, making it impossible to map them to the warped image such that the chess squares are snapped to the grid of whole numbers.
Instead, we will randomly choose four points that we believe to form a rectangle on the chessboard.
This means that we will select two horizontal and two vertical lines at random, and choose the four intersection points of these lines as the random sample.
...
However, since the set of inliers will likely not contain all vertices of the chessboard squares (for example because some lines were not recognised as seen in \cref{fig:board_localisation_line_detection_elimination,fig:incorrect_intersection_points}) we do not yet know the origin of that $8 \times 8$.

Once we have computed the output of \cref{alg:ransac} and obtained the mapping $m$ of points from the original image to points on the grid of the warped image, we can recompute the homography matrix $\mH$ using \cref{eq:homography_matrix_relation_corners} using \emph{all} inliers to obtain a more accurate homography that is based on more than just four points.
To do so, we take the matrix of original points $\mP$ to be a $3 \times \abs{\sI}$ matrix whose columns are populated by all the intersection points $\sI$ as homogenous coordinate vectors.
...
Using the new $\mP$ and $\mP'$ matrices, \cref{eq:homography_matrix_relation_corners} becomes an overdetermined linear system, so we will compute $\mH$ as the least squared error solution instead of the exact solution.
...
\begin{itemize}
    ...
    \item The for loops on lines \ref{alg:ransac:for_sx} and \ref{alg:ransac:for_sy} do not need to be nested; instead, one could first consider only the $x$-component of the vectors when calculating the errors for each scale $s_x$, and then do the same for the $y$-components and $s_y$.
        Finally, we would choose the best $s_x$ and $s_y$ (with the least accumulated error), compute the joint error, and use the information to update the mappings if necessary.
...
It was noted at the end of \cref{sec:find_optimal_homography} that while the result of \cref{alg:ransac} projects the inlier intersection points onto a regular grid of whole numbers, that result does not explicitly indicate the location of the $8\times 8$ sub-grid that represents the chessboard.
...
\begin{caselist}
    ...
    \begin{case}{$x_\text{max}-x_\text{min}>8$}
        This case arises when the vertical line $x=x_\text{min}$ is left of the left edge of the chessboard and/or the vertical line $x=x_\text{max}$ is right of the right edge of the chessboard.
    ...
    \begin{case}{$x_\text{max}-x_\text{min}<8$}\label{case:infer_borders_greater}%
        ...
        \Cref{fig:homography_warped} illustrates this case makes it clear that it arises because either one or both of the outermost vertical lines were not detected on the chessboard. 
        ...
        Typically, the results of both convolution operations is calculated, and then some simple trigonometry can be applied in order to determine the gradient directions for each pixel.
        ...
        \begin{equation}
            ...
            \mG_x = \mS_x * \mA
        \end{equation}
        where $\mG_x$ is of the same dimensionality as $\mA$ and contains the gradient intensities.
...
After following the instructions according to the three cases above for both the horizontal and vertical directions, we will have obtained the
...
The method of finding the four corner points of the chessboard described in the previous sections relies on numerous parameters whose optimal values are yet to be determined.
...
\end{itemize}
The optimal parametervalues are decided by performing a grid search using sensible presets.
...
On the entire training set of 4,400 samples, the corner detection algorithm yielded inaccurate predictions in 60 cases, so its accuracy is 98.64\%.
The validation set of size 146 sees only 2 mistakes and thus achieves an accuracy of 98.63\%.
These numbers are quite similar, thus we conclude that the corner detection algorithm is very robust to unseen samples from the dataset and did not overfit as a result of the grid search.
