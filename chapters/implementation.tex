...
More details follow in \cref{sec:user_man_recap_implementation}, and comprehensive documentation\footnote{The documentation is automatically published to \url{https://recap.readthedocs.io} via a \gls{cd} pipeline as indicated in \cref{fig:cicd}.} of the \texttt{recap} library is available at \url{https://recap.readthedocs.io}.
...
\end{enumerate*}
React apps are typically run using Node, but since all the code client-side, we bundle the app to a set of static \gls{html} files that are served using \texttt{nginx}.
...
\end{figure}
The architecure is quite basic and is not optimised for scale because its purpose is merely to provide a proof of concept.
...
The \gls{dns} provider for the \texttt{www.chesscog.com} domain has a \gls{cname} record that points to \texttt{chesscog.herokuapp.com}\footnote{Technically, the \gls{cname} record points at a Heroku \gls{dns} server that forwards to \texttt{chesscog.herokuapp.com}.}, so the website is accessible at \texttt{https://www.chesscog.com}.
