...
\begin{figure}
    ...
    \caption[Overview of \acs{ci}/\acs{cd} pipelines.]{Overview of \acs{ci}/\acs{cd} pipelines. Trapeziums represent actions initiated by the developer, rectangles correspond to automated processes executed by the pipeline, and the thick arrows represent logical dependency. The current status of each pipeline is available at \path{https://github.com/georgw777/{repo}/actions} where \texttt{\{repo\}} is to be substituted with the repository’s name.}
    \label{fig:cicd}
\end{figure}
...
It provides one main route\footnote{There is an interactive \gls{api} documentation available at \url{https://www.chesscog.com/api/docs}.}, \path{/api/predict}, which upon receiving a \gls{http} POST request with an input image, runs the chess recognition pipeline and returns the predicted \gls{fen} position alongside some other information in \gls{json} format.
